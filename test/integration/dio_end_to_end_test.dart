...
void main() {
  group('Dio End-to-End Integration Tests', () {
    ...
    group('Provider Builder HTTP Configuration', () {
      test('should apply HTTP configuration through LLMBuilder for Anthropic', () async {
        ...
        final httpLogs = logRecords.where((record) => 
          record.loggerName == 'HttpConfigUtils' && 
          record.message.contains('→ POST')).toList();
        expect(httpLogs.length, greaterThan(0));

        // Should have custom header in logs
        final headerLogs = logRecords.where((record) => 
          record.message.contains('X-Test-Client')).toList();
        expect(headerLogs.length, greaterThan(0));
      });

      test('should apply HTTP configuration through LLMBuilder for OpenAI', () async {
        // Clear any setup logs
        logRecords.clear();
      ...
      test('should apply HTTP configuration through LLMBuilder for DeepSeek', () async {
        // Clear any setup logs
        logRecords.clear();
      ...
      test('should apply HTTP configuration through LLMBuilder for Groq', () async {
        // Clear any setup logs
        logRecords.clear();
      ...
      test('should apply HTTP configuration through LLMBuilder for xAI', () async {
        // Clear any setup logs
        logRecords.clear();
      ...
      test('should apply HTTP configuration through LLMBuilder for Google', () async {
        // Clear any setup logs
        logRecords.clear();
    ...
    group('Complex HTTP Configuration Scenarios', () {
      test('should handle comprehensive HTTP configuration', () async {
        ...
        final versionLogs = logRecords.where((record) => 
          record.message.contains('X-Test-Version')).toList();
        expect(versionLogs.length, greaterThan(0));

        final envLogs = logRecords.where((record) => 
          record.message.contains('X-Test-Environment')).toList();
      ...
      test('should work without HTTP configuration', () async {
        ...
        final httpLogs = logRecords.where((record) => 
          record.loggerName == 'HttpConfigUtils').toList();
      ...
      test('should handle streaming with HTTP configuration', () async {
        ...
        final streamLogs = logRecords.where((record) => 
          record.message.contains('X-Test-Stream')).toList();
        expect(streamLogs.length, greaterThan(0));
      });
    });

    group('Error Scenarios with HTTP Configuration', () {
      test('should log errors properly with HTTP configuration enabled', () async {
        // Clear any setup logs
        logRecords.clear();
        ...
        final headerLogs = logRecords.where((record) => 
          record.message.contains('X-Test-Error')).toList();
        expect(headerLogs.length, greaterThan(0));
      });
    });
  });
