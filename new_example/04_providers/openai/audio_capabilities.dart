...
Future<void> testTextToSpeech(AudioCapability provider) async {
  ...
  try {
    ...
    print('   ✅ Basic TTS: ${basicTTS.audioData.length} bytes → openai_basic.mp3');
    ...
    print('   ✅ Dramatic TTS: ${dramaticTTS.audioData.length} bytes → openai_dramatic.wav');
    
    // Test convenience method
    final quickSpeech = await provider.speech('Quick test using convenience method');
...
Future<void> testSpeechToText(AudioCapability provider) async {
  ...
  try {
    ...
    if (await File('openai_basic.mp3').exists()) {
      ...
      // Test convenience method
      final quickTranscription = await provider.transcribeFile('openai_basic.mp3');
